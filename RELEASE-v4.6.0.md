...
- 新增任务中心：记录记忆去重、画像生成、模型检测、候选记忆批量审核等耗时管理动作，支持按关键词、类型、状态和当前群/全部群范围筛选。
- 长任务写入任务记录：去重应用、画像生成/刷新、模型连接检测、候选批量审核会生成执行状态、进度、耗时和结果摘要。
- 新增模型健康历史：系统状态页展示最近模型检测来源、结果、延迟和检测时间；总览/健康页检测会写入历史，并支持展开查看模型 ID、分类、缓存状态、服务地址和完整错误详情。
...
- 管理端截图巡检覆盖 `/tasks`、`/tasks` 详情、`/audit`、`/audit` 详情与 `/health` 模型详情，并更新静态测试锁定 V4.6.0 版本标识、任务中心、操作审计、模型健康历史详情和排期预览入口。
