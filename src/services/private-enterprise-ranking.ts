...
export class PrivateEnterpriseRanking {
  ...
  answer(text: string, history: Array<{ role: string; content: string }> = []): RankingAnswer | undefined {
    ...
    if (years.some((year) => year !== 2026)) {
      return /民营|民企|500强/.test(text) && /排名|名次|第几|多少家|几家|哪些|名单|分别是/.test(text)
        ? { messages: ["目前知识库仅收录2026中国民营企业500强，不能用这份榜单回答其他年份的名次或地区统计。"] }
        : undefined;
    }

    const scope = /民营|民企|500强|2026/.test(text);
    const rankIntent = /排名|名次|排第几|第几名|第几位|多少名|排多少|位列|第\s*\d+\s*名/.test(text);
    const listIntent = /哪些|哪几家|分别是|名单|有谁|都有哪些/.test(text);
    const countIntent = /多少家|几家|上榜|入围/.test(text);
    const prior = history.at(-1)?.role === "assistant" && history.at(-1)?.content.includes(EDITION)
      ? [...history].reverse().find((turn) => turn.role === "user")?.content
      : undefined;
    const followup = !scope && /^(?:分别是哪些|有哪些|都有哪些|哪几家|名单)(?:[？?。])?$/.test(text.trim());
    const question = followup && prior ? prior : text;
    const provinceMatches = this.matchProvinces(question);
    ...
    if (!scope && !followup && !(provinceMatches.length > 0 && (rankIntent || countIntent || listIntent)) &&
        !(rankIntent && companyMatches.length > 0)) {
      return undefined;
    }
    if (!rankIntent && !listIntent && !countIntent && !followup) return undefined;

    if (rankIntent || (countIntent && /有无|有没有|是否|哪家|公司|企业/.test(text) && !/多少家|几家/.test(text))) {
      const exactRank = text.match(/第\s*(\d{1,3})\s*名/);
      if (exactRank && /谁|哪家|什么企业/.test(text)) {
        const entry = this.data.entries[Number(exactRank[1]) - 1];
        return { messages: [entry
          ? `${EDITION}第${entry.rank}名是${entry.name}（榜单省份：${entry.province}）。`
          : "榜单名次应在1至500之间。"] };
      }
  ...
  private matchEnterprises(text: string): RankedEnterprise[] {
    ...
    const candidate = normalize(text.replace(/20\d{2}年?|中国|全国|民营企业|民企|500强|排名|名次|排第几|第几名|第几位|多少名|排多少|位列|在|的|是|多少|几|第|名|请问|帮我|查一下|告诉我|上榜|入围|了吗|吗|呢|公司|企业|[？?！!。，、\s]/g, ""));
