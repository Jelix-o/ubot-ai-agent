...
async function renderHealth() {
  const token = nextRenderToken();
  setPageLoading('正在读取系统状态...');
  const data = await apiForRender('/api/health');
  if (!data || !isLatestRender(token)) return;
  content().innerHTML = '<section class="panel"><div class="section-head"><div><h2>系统状态</h2><p>模型异常时会暂停画像、记忆或消息生成；服务器状态用于排查运行环境。</p></div><button type="button" data-refresh-health>立即检测</button></div><div class="health-grid">' + healthCardHtml('NapCat 连接', data.transportHealth) + healthCardHtml('模型检测', data.profileAiHealth) + healthCardHtml('Node 运行时', { ok: true, detail: data.nodeVersion + ' · PID ' + data.pid, checkedAt: 'uptime ' + data.uptimeSeconds + 's' }) + healthCardHtml('内存占用', memoryHealthStatus(data.memory)) + '</div><details class="health-diagnostics"><summary>查看原始诊断 JSON</summary><pre>' + esc(JSON.stringify(data, null, 2)) + '</pre></details></section>';
}
