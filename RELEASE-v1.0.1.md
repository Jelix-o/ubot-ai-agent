...
发布状态：本地实现、构建、全量测试、本地后台截图验证、Windows 发布包和 GitHub Release dry-run 已完成。按用户最新“改完先不用部署”要求，暂不执行生产部署、push、tag 推送、GitHub Release 上传和关机。
...
- 普通用户后台会根据当前 NapCat 群成员数据计算可见群，只显示这个 QQ 所在的已启用群。
...
- `npm run build`：通过。
- `node scripts/run-node22.cjs --test --experimental-test-isolation=none dist\admin-http-server.test.js`：2/2 通过，覆盖普通 QQ 只读登录、可读范围、禁止跨群、禁止写操作和 `readonly_session`。
- `npm test`：352/352 通过。
- `git diff --check`：通过。
- `$env:ADMIN_SMOKE_SCREENSHOTS="1"; node scripts/run-node22.cjs scripts/visual-admin-smoke.mjs`：通过，生成 29 张后台截图和 contact sheet。
