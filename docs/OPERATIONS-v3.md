...
1. Download the matching `ubot-3.0.6-linux.tar.gz` and `ubot-3.0.6-linux.tar.gz.sha256` assets from the GitHub Release, then validate the archive with that manifest. Do not substitute a locally built or differently tagged bundle.
...
1. Leave all UBot services stopped and retain `/opt/ai-project/release-backups/<timestamp>-v3.0.6` plus `data/v3-rollback` and `data/generated-pages`.
