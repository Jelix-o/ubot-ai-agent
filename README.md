...
- `data/iteration-feedback.json`：自我迭代反馈池
- `data/iteration-plans.json`：自我迭代开发计划
...
- `#迭代 反馈 <内容>` 记录当前群的行为、数据、技能、模型、功能或运维反馈；超级管理员可用 `#迭代 计划` 生成可交给 `/goal` 的开发计划。
...
- v4.7.1 后台默认使用分页、筛选摘要、URL 状态恢复、来源证据按需加载、路由懒加载、精简列表、模型健康历史、任务中心、操作审计、自我迭代和分组选择隔离；候选记忆、长期记忆默认只显示核心信息，详情和危险操作按需展开，降低大列表卡顿。
- 后台“自我迭代”页面可汇总 QQ 命令和后台录入的反馈，读取群配置、候选记忆、知识库、技能、模型健康和操作日志，生成可复制到 `/goal` 模式的开发计划；审批后的计划只会标记低风险反馈完成，不会在线上直接改源码。
