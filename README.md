...
- `#昨日画像 <备注/QQ号>` 查看自己或管理员指定成员昨日新增画像总结；省略参数默认查自己。
- `#群聊画像 <备注/QQ号>` 汇总自己或管理员指定成员的整体长期群聊画像；省略参数默认查自己。
...
- 自动提炼的记忆置信度达到 0.6 且满足归属规则时会自动批准进入长期记忆；低置信度或缺少成员归属的画像仍留在候选池。
...
- `#昨日画像` 和 `#群聊画像` 支持用 QQ、系统备注、别名、群名片或昵称查询；普通成员只能查自己，管理员可以查任意成员，多人模糊命中时会提示改用 QQ 号。
